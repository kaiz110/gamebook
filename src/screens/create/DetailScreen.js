...
import { ADD_BG_IMAGE, DEL_PAGE } from '../../lib/redux/actions/createActions'
...
const DetailScreen = ({navigation,route}) => {
    ...
    const listHeader = () => (
        <View>
            {book.bimage != '' &&
            <Image 
                source={{uri: `data:image/jpeg;base64,${book.bimage}`}} 
                style={styles.bimage}/>
            }
            ...
            <View style={{flexDirection: 'row'}}>
                <View>
                    <TouchableOpacity 
                        ...
                        onPress={async () => {
                            const resultImage = await openImage(true)
                            if(resultImage != null) dispatch(ADD_BG_IMAGE(resultImage))
                        }}
                    >
            ...
            <View style={styles.overlayCon}>
                ...
                <View style={styles.overlayBt}>
                    <Button 
                        title={TEXT.yes}
                        type='clear'
                        disabled={book.error.length > 0 || book.story.length < 1}
                        onPress={() => {
